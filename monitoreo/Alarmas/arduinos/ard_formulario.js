	...
	if (ingrediente.val() > 0){
		...
		html += '<tr>';
			...
			html += '<td class="col-xs-4">';
				html += '<input type="checkbox" name="activo_sens[]" value="0">';
				html += '<input type="hiden"  readonly name="sensores[]" value="' + ingrediente.val() + '">';
				 // <input type="checkbox" name="activo_sens[]" value="<?php echo $value['activo'] ?>">
				// <input type="hidden" readonly value="<?php echo $value['id_sensores'] ?>" name="sensores[]" class="form-control">
			html += '</td>';
		html += '</tr>';
